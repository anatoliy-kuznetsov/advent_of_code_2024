        ...
        for i, line in enumerate(maze_text):
            for j, char in enumerate(line):
                ...
                if (i, j) == (1, 1):
                    ...
                    continue
                if (i, j) == (self.height - 2, self.width - 2):
                    self.cells_along_path.add((i, j))
                    self.ends = [
                        ((i, j), "N"),
                        ((i, j), "E"),
...
def simulate_falling_bytes(byte_coordinates: list[str], nanoseconds: int, dimensions: tuple[int, int]) -> list[str]:
    """
    Retuns maze as list of lines
    """
    obstacle_locations = set()
    for i in range(nanoseconds):
        tokens = byte_coordinates[i].split(",")
        obstacle_locations.add((int(tokens[0]) + 1, int(tokens[1]) + 1))
    height = dimensions[0]
    width = dimensions[1]
    for i in range(height):
        obstacle_locations.add((i, 0))
        obstacle_locations.add((i, width - 1))
    for j in range(width):
        obstacle_locations.add((0, j))
        obstacle_locations.add((height - 1, j))
    return ["".join(
        ["#" if (i, j) in obstacle_locations else "." for j in range(width)])
        for i in range(height)]

with open("input.txt", "r") as f:
    lines = f.readlines()
maze_lines = simulate_falling_bytes(lines, 1024, (Maze.height, Maze.width))
maze = Maze(maze_lines)
shortest_path_length = maze.solve()
print(f"Shortest path length is {shortest_path_length}")

max_obstacles = len(lines)
min_obstacles = 1024
while max_obstacles > min_obstacles + 1:
    obstacle_count = (min_obstacles + max_obstacles) // 2
    print(f"Trying {obstacle_count} obstacles")
    maze_lines = simulate_falling_bytes(lines, obstacle_count, (Maze.height, Maze.width))
    maze = Maze(maze_lines)
    shortest_path_length = maze.solve()
    if shortest_path_length == Maze.MAX_DISTANCE:
        max_obstacles = obstacle_count
        print(f"\tno path to end, new bounds: {min_obstacles}, {max_obstacles}")
    else:
        min_obstacles = obstacle_count
        print(f"\tfound path to end, new bounds: {min_obstacles}, {max_obstacles}")

print(lines[min_obstacles])